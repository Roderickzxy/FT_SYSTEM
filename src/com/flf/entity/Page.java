...
public class Page {
	...
	private int currentCount;	//当前页的记录数
	private boolean entityOrField;	//true:需要分页的地方，传入的参数就是Page实体；false:需要分页的地方，传入的参数所代表的实体拥有Page属性
	private String pageStr;		//最终页面显示的底部翻页导航，详细见：getPageStr();
	private String ajaxPageStr;//前端ajax方式翻页代码
	public int getTotalPage() {
		if(getTotalResult()%showCount==0)
			totalPage = getTotalResult()/showCount;
		else
			totalPage = getTotalResult()/showCount+1;
		return totalPage;
	}
	...
	public int getCurrentCount() {
		...
		{
			if(getTotalResult()<=getShowCount())
			{
				return getTotalResult();
			}else
			{
				return getTotalResult()-((getCurrentPage()-1)*getShowCount());
			}
	...
	public String getAjaxPageStr() {
		...
		if(getTotalResult()>0){
			...
			if(getCurrentPage()==getTotalPage()){
				...
				sb.append("	<li><a href=\"#@\" onclick=\"nextPage("+(getCurrentPage()+1)+")\">下页</a></li>\n");
				sb.append("	<li><a href=\"#@\" onclick=\"nextPage("+getTotalPage()+")\">尾页</a></li>\n");
			}
			sb.append("	<li class=\"pageinfo\">第"+getCurrentPage()+"页</li>\n");
...
//			sb.append("var json = {};\n");
			sb.append("var jsonPage = {currentPage :page};\n");
			sb.append("json[\"page\"] = jsonPage;\n;");
 			sb.append("ajaxPage(json);\n");
			sb.append("}\n");
			sb.append("</script>\n");
		}
	...
	public String toString() {
		return "Page [showCount=" + showCount + ", totalPage=" + totalPage
				...
				+ ", currentCount=" + currentCount + ", entityOrField="
				+ entityOrField + ", pageStr=" + pageStr + "]";
	}
